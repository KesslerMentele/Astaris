...
public class CometProjectile : ModProjectile
{
    ...
    private const string ChainTexturePath = "Astaris/Content/Projectiles/Weapons/CometChain"; // The folder path to the flail chain sprite
    private const string ChainTextureExtraPath = "Astaris/Content/Projectiles/Weapons/CometChainExtra";  // This texture and related code is optional and used for a unique effect
    ...
    public override void AI()
    {
        ...
        switch (CurrentAIState)
        {
            case AIState.Spinning:
                ...
                {
                    Vector2 unitVectorTowardsMouse = mountedCenter.DirectionTo(Main.MouseWorld)
            ...
            case AIState.Retracting:
            {
                ...
                else
                {
                    ...
                    Projectile.velocity = Projectile.velocity.MoveTowards(unitVectorTowardsPlayer 
                                                                          * maxRetractSpeed, retractAcceleration);
                    player.ChangeDir((player.Center.X < Projectile.Center.X).ToDirectionInt());
                }

                break;
            }
            case AIState.UnusedState: // Copied from ExampleMod, just in case
            {
                if (!player.controlUseItem) {
                ...
                if (currentChainLength > unusedChainLength) {
                    ...
                    if (currentChainLength >= launchRange) {
                        ...
                        Projectile.velocity = Projectile.velocity.MoveTowards(Projectile
                            .DirectionTo(mountedCenter).SafeNormalize(Vector2.Zero) * unusedMaxRetractSpeed, 
                            unusedMaxRetractSpeed);
                    }
                    Projectile.velocity *= 0.98f;
                    Projectile.velocity = Projectile.velocity.MoveTowards(Projectile
                        .DirectionTo(mountedCenter).SafeNormalize(Vector2.Zero) * unusedMaxRetractSpeed, 
                        unusedRetractAcceleration);
                }
                else {
            ...
            case AIState.ForcedRetracting:
            {
                ...
                Projectile.velocity *= 0.98f;
                Projectile.velocity = Projectile.velocity.MoveTowards(unitVectorTowardsPlayer * 
                                                                      maxForcedRetractSpeed, forcedRetractAcceleration);
        ...
        if (Main.rand.NextBool(dustRate))
            Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<RockCrystalDust>(), 0f, 0f, 150, default(Color), 1.3f);
    }
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        if (CurrentAIState == AIState.LaunchingForward)
        {
            foreach (Vector2 direction in roseVecs)
            {
                Projectile.NewProjectileDirect(
                    Terraria.Entity.InheritSource(Projectile), 
                    Projectile.position, 
                    direction * 15.0f, 
                    ModContent.ProjectileType<RockCrystalBulletProjectile>(), 
                    Projectile.damage, 
